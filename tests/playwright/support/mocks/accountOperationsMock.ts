 ...
 *
 * The account search fires two live-backend requests that historically made
 * these tests flaky:
 *   1. the account-name autocomplete (`.../input-type/<name>%`)
 *   2. the operations query (`.../<account>/operations?operation-types=...`)
 *
 * On a slow backend the operations query can exceed Playwright's default
 * response timeout, so instead of waiting on the live response we fulfil both
 * requests with fixed fixtures. This keeps the UI assertions meaningful
 * (results card renders, "vote" rows show) while removing all backend latency.
 */
