...
// Windows are contiguous and non-overlapping: (0,30], (30,90], (90,180].
const RETENTION_COLS = [
  ...
  {
    key: "pct_180d" as const,
    countKey: "active_at_180d" as const,
    label: "91–180D",
    xIdx: 3,
  },
];
...
interface NetworkAuthorRetentionHeatmapProps {
  data: Hive.NetworkAuthorRetentionResponse[];
  compact?: boolean;
...
const NetworkAuthorRetentionHeatmap: React.FC<
  NetworkAuthorRetentionHeatmapProps
> = ({ data, compact = false, viewMode = "rates", showLegend = false }) => {
  const { t, locale, dir } = useI18n();
  const isRTL = dir === "rtl";
  const { theme } = useTheme();
  const isDark = theme === "dark";

  const tActive = t("networkAuthorRetentionCard.heatmapActive");
  const tAuthors = t("networkAuthorRetentionCard.heatmapAuthors");
  const tNewAuthors = t("networkAuthorRetentionCard.newAuthors");
  const tDataPending = t("networkAuthorRetentionCard.heatmapDataPending");
  ...
  const { minNew, maxNew } = useMemo(() => {
    ...
    const vals = data.map((r) => r.first_post_accounts);
  ...
  const { minActive, maxActive } = useMemo(() => {
    ...
    const vals = data.flatMap((r) =>
      [r.active_at_30d, r.active_at_90d, r.active_at_180d].filter(
  ...
  const seriesData = useMemo(() => {
    ...
    return data.flatMap((row, yIdx) => {
      ...
      const tNorm = (row.first_post_accounts - minNew) / newRange;
      const newCell = {
        value: [NEW_COL_IDX, yIdx, row.first_post_accounts],
  ...
  const option = useMemo(() => {
    const xAxisLabels = [tNewAuthors, ...RETENTION_COLS.map((c) => c.label)];
      ...
      tooltip: {
        ...
        formatter: (params: { value: [number, number, number] }) => {
          ...
          if (xIdx === NEW_COL_IDX) {
            return `
              <div style="line-height:1.6;min-width:160px">
                ...
                <div style="font-size:11px">${tNewAuthors}: <strong>${rawVal.toLocaleString(locale)}</strong></div>
              </div>`;
          }
          ...
          if (viewMode === "counts") {
            ...
            const newLine = `<div style="font-size:10px;color:${mutedColor};margin-top:1px">${tNewAuthors}: <strong style="color:${textColor}">${row.first_post_accounts.toLocaleString(locale)}</strong></div>`;
            return `
              <div style="line-height:1.6;min-width:160px">
                <div style="font-weight:700;font-size:12px;margin-bottom:3px">${monthLabel}</div>
                ${newLine}
                <div style="font-size:11px;margin-top:4px">${col.label}: <strong>${countText}</strong> ${tAuthors}</div>
              </div>`;
          }
          ...
          const countLine =
            activeCount !== null
              ? `<div style="font-size:10px;color:${mutedColor};margin-top:2px">${activeCount.toLocaleString(locale)} ${tAuthors}</div>`
              : rawVal >= 0
                ? ""
          ...
          return `
            <div style="line-height:1.6;min-width:160px">
              <div style="font-weight:700;font-size:12px;margin-bottom:3px">${monthLabel}</div>
              ${newLine}
              <div style="font-size:11px;margin-top:4px">${col.label} ${tActive}: <strong>${pctText}</strong></div>
              ${countLine}
            </div>`;
        },
...
export default NetworkAuthorRetentionHeatmap;
