 ...
 */
const scaleValueLog = (value: number, from: [number, number], to: [number, number]): number => {
    if (from[1] <= from[0]) return to[0];
...
const getChartOption = (
  ...
  const seriesConfig = {
    ...
    edgeSymbolSize: 10,
    lineStyle: { color: 'source', opacity: 0.8, curveness: isFullyExpanded ? 0 : 0.1 },
    color: ['#ef4444', '#10b981', '#34d399', '#3b82f6', '#60a5fa'],
    ...(isFullyExpanded
      ? { layout: 'none' }
      : { layout: 'force', force: { repulsion: 600, edgeLength: 120, gravity: 0.1 } }
    ),
  };
  ...
  return {
    tooltip: {
      formatter: (params: any) => {
        if (params.dataType === 'edge') return `${t('influenceMapReport.tooltipDelegationPrefix')}${params.value}`;
...
const InfluenceMapReport: React.FC<InfluenceMapReportProps> = ({ 
  ...
  const [fullIncoming, setFullIncoming] = useState<Explorer.VestingDelegation[]>([]);
  const [fullOutgoing, setFullOutgoing] = useState<Explorer.VestingDelegation[]>([]);
  ...
  const [dataCache, setDataCache] = useState(new Map<string, { incoming: Explorer.VestingDelegation[], outgoing: Explorer.VestingDelegation[] }>());
 
  // State to track which nodes have been clicked and expanded in the interactive view.
  const [expandedNodes, setExpandedNodes] = useState(new Set<string>([accountName]));
 
  // State to manage pagination for nodes with many connections ('Load More' functionality).
  const [nodePages, setNodePages] = useState(new Map<string, { incoming: number, outgoing: number }>());
  ...
  const [activeLegendCategories, setActiveLegendCategories] = useState<{ name: string }[]>([]);
  ...
  const fetchedOutgoing = useConvertedVestingShares("outgoing", accountToFetch || '', liveDataEnabled, dynamicGlobalData, !!accountToFetch);
  const fetchedIncoming = useConvertedVestingShares("incoming", accountToFetch || '', liveDataEnabled, dynamicGlobalData, !!accountToFetch);
  ...
  useEffect(() => {
    if (accountToFetch && (fetchedIncoming || fetchedOutgoing)) {
      setDataCache(prevCache => new Map(prevCache).set(accountToFetch, {
...
useEffect(() => {
    ...
    const resizeObserver = new ResizeObserver(() => {
        ...
        setChartDimensions({ width: container.offsetWidth, height: container.offsetHeight });
    });
  ...
  useEffect(() => {
    const sortedIncoming = data.incoming ? [...data.incoming].sort((a, b) => parseFloat(b.amount) - parseFloat(a.amount)) : [];
    const sortedOutgoing = data.outgoing ? [...data.outgoing].sort((a, b) => parseFloat(b.amount) - parseFloat(a.amount)) : [];
    
    setFullIncoming(sortedIncoming);
    setFullOutgoing(sortedOutgoing);
    setIsFullyExpanded(false);
    setDataCache(new Map([[accountName, { incoming: sortedIncoming, outgoing: sortedOutgoing }]]));
    ...
    setNodePages(new Map([[accountName, { incoming: initialPageCount, outgoing: initialPageCount }]]));
  ...
  const generateGraph = useCallback(() => {
      ...
      if (isFullyExpanded) {
        ...
        const amounts = allDelegations.map(d => parseFloat(d.amount)).filter(a => a > 0);
        const minAmount = amounts.length > 0 ? Math.min(...amounts) : 0;
        const maxAmount = amounts.length > 0 ? Math.max(...amounts) : 0;
        
        nodes.push({ id: centralNodeId, name: `@${accountName}`, symbolSize: 60 * mobileSizeMultipler, category: 0, value: t('analyticsDashboard.centralHub') });
        ...
        if ((fullIncoming.length + fullOutgoing.length) > EXPAND_ALL_NODE_LIMIT) {
            ...
            const finalIncomingLimit = Math.min(fullIncoming.length, actualIncomingCount + (slotsPerSide - actualOutgoingCount));
            const finalOutgoingLimit = Math.min(fullOutgoing.length, actualOutgoingCount + (slotsPerSide - actualIncomingCount));
        ...
        let totalNodesToPosition = incomingToShow.length + outgoingToShow.length + (hasIncomingOthers ? 1 : 0) + (hasOutgoingOthers ? 1 : 0);
        let currentIndex = 0;
        
        incomingToShow.forEach((delegation) => {
            const amount = parseFloat(delegation.amount);
            const radius = currentIndex % 3 === 0 ? MAX_EDGE_LENGTH : currentIndex % 3 === 1 ? MIN_EDGE_LENGTH : (MIN_EDGE_LENGTH + MAX_EDGE_LENGTH) / 2;
            const angle = totalNodesToPosition > 0 ? (currentIndex / totalNodesToPosition) * 2 * Math.PI : 0;
            nodes.push({ id: `in-${delegation.delegator}`, name: `@${delegation.delegator}`, symbolSize: scaleValueLog(amount, [minAmount, maxAmount], [MIN_SYMBOL_SIZE * mobileSizeMultipler, MAX_SYMBOL_SIZE * mobileSizeMultipler]), category: 1, value: delegation.vesting_shares, amount, x: centerX + radius * Math.cos(angle), y: centerY + radius * Math.sin(angle), fixed: true });
            links.push({ source: `in-${delegation.delegator}`, target: centralNodeId, value: delegation.vesting_shares, amount, lineStyle: { width: scaleValue(amount, [minAmount, maxAmount], [0.5, 8]) } });
            currentIndex++;
        });

        if (hasIncomingOthers) {
            const remaining = fullIncoming.slice(incomingToShow.length);
            const sumAmount = remaining.reduce((sum, d) => sum + parseFloat(d.amount), 0);
            const sumHP = remaining.reduce((sum, d) => sum + grabNumericValue(d.vesting_shares), 0);
            const radius = currentIndex % 3 === 0 ? MAX_EDGE_LENGTH : currentIndex % 3 === 1 ? MIN_EDGE_LENGTH : (MIN_EDGE_LENGTH + MAX_EDGE_LENGTH) / 2;
            const angle = totalNodesToPosition > 0 ? (currentIndex / totalNodesToPosition) * 2 * Math.PI : 0;
            nodes.push({ id: 'in-others-expanded', name: `${remaining.length}+ ${t('analyticsDashboard.others')}`, symbolSize: scaleValueLog(sumAmount, [minAmount, maxAmount], [MIN_SYMBOL_SIZE * mobileSizeMultipler, (MAX_SYMBOL_SIZE + 5) * mobileSizeMultipler]), category: 2, value: `${remaining.length} ${t('influenceMapReport.moreDelegations')} (${sumHP.toLocaleString()} HP)`, x: centerX + radius * Math.cos(angle), y: centerY + radius * Math.sin(angle), fixed: true });
            links.push({ source: 'in-others-expanded', target: centralNodeId, value: `${sumHP.toLocaleString()} HP`, lineStyle: { width: scaleValue(sumAmount, [minAmount, maxAmount], [0.5, 8]) } });
            currentIndex++;
        }
        ...
        outgoingToShow.forEach((delegation) => {
            ...
            const radius = currentIndex % 3 === 0 ? MAX_EDGE_LENGTH : currentIndex % 3 === 1 ? MIN_EDGE_LENGTH : (MIN_EDGE_LENGTH + MAX_EDGE_LENGTH) / 2;
            const angle = totalNodesToPosition > 0 ? (currentIndex / totalNodesToPosition) * 2 * Math.PI : 0;
            nodes.push({ id: `out-${delegation.delegatee}`, name: `@${delegation.delegatee}`, symbolSize: scaleValueLog(amount, [minAmount, maxAmount], [MIN_SYMBOL_SIZE * mobileSizeMultipler, MAX_SYMBOL_SIZE * mobileSizeMultipler]), category: 3, value: delegation.vesting_shares, amount, x: centerX + radius * Math.cos(angle), y: centerY + radius * Math.sin(angle), fixed: true });
            links.push({ source: centralNodeId, target: `out-${delegation.delegatee}`, value: delegation.vesting_shares, amount, lineStyle: { width: scaleValue(amount, [minAmount, maxAmount], [0.5, 8]) } });
            currentIndex++;
        });
        ...
        if (hasOutgoingOthers) {
            ...
            const radius = currentIndex % 3 === 0 ? MAX_EDGE_LENGTH : currentIndex % 3 === 1 ? MIN_EDGE_LENGTH : (MIN_EDGE_LENGTH + MAX_EDGE_LENGTH) / 2;
            const angle = totalNodesToPosition > 0 ? (currentIndex / totalNodesToPosition) * 2 * Math.PI : 0;
            nodes.push({ id: 'out-others-expanded', name: `${remaining.length}+ ${t('analyticsDashboard.others')}`, symbolSize: scaleValueLog(sumAmount, [minAmount, maxAmount], [MIN_SYMBOL_SIZE * mobileSizeMultipler, (MAX_SYMBOL_SIZE + 5) * mobileSizeMultipler]), category: 4, value: `${remaining.length} ${t('influenceMapReport.moreDelegations')} (${sumHP.toLocaleString()} HP)`, x: centerX + radius * Math.cos(angle), y: centerY + radius * Math.sin(angle), fixed: true });
            links.push({ source: centralNodeId, target: 'out-others-expanded', value: `${sumHP.toLocaleString()} HP`, lineStyle: { width: scaleValue(sumAmount, [minAmount, maxAmount], [0.5, 8]) } });
        }
        finalNodes = nodes;
        finalLinks = links;
      } else {
          ...
          const allDelegations = Array.from(dataCache.values()).flatMap(d => [...d.incoming, ...d.outgoing]);
          const amounts = allDelegations.map(d => parseFloat(d.amount)).filter(a => a > 0);
          ...
          while (queue.length > 0) {
              ...
              if (!nodeMap.has(parentAccountName)) {
                  ...
                  nodes.push({
                      ...
                      name: isLoading ? `${t('influenceMapReport.legendLoading')}...` : `@${parentAccountName}`,
                      symbolSize: isRoot ? rootNodeSize : childNodeSize,
                      category: isRoot ? 0 : nodeMap.get(parentAccountName)?.category || 4,
                      value: isRoot ? t('analyticsDashboard.centralHub') : '',
                      label: { show: isRoot || expandedNodes.has(parentAccountName) },
                      ...(isRoot && chartDimensions.width > 0 && chartDimensions.height > 0 && { x: chartDimensions.width / 2, y: chartDimensions.height / 2, fixed: true }),
                      ...(position && !isRoot && { x: position.x, y: position.y, fixed: true }),
                  });
                  nodeMap.set(parentAccountName, nodes[nodes.length - 1]);
              }

              if (!expandedNodes.has(parentAccountName) || !dataCache.has(parentAccountName)) continue;
              
              visited.add(parentAccountName);
              const { outgoing, incoming } = dataCache.get(parentAccountName)!;
              const pages = nodePages.get(parentAccountName) || { incoming: 1, outgoing: 1 };
              ...
              outgoingToShow.forEach(delegation => {
                  ...
                  if (!nodeMap.has(childAccountName)) {
                      ...
                      nodes.push({ 
                          id: childAccountName, name: `@${childAccountName}`, symbolSize: childNodeSize, category: 3, value: delegation.vesting_shares,
                  ...
                  links.push({ source: parentAccountName, target: childAccountName, value: delegation.vesting_shares, lineStyle: { width: scaleValue(parseFloat(delegation.amount), [minAmount, maxAmount], [1, 8]) } });
              });
              ...
              if (outgoing.length > outgoingToShow.length) {
                  ...
                  nodes.push({ id: othersId, name: `${remaining.length}+ ${t('analyticsDashboard.others')}`, symbolSize: scaleValue(sumAmount, [minAmount, maxAmount], [10 * mobileSizeMultipler, 25 * mobileSizeMultipler]), category: 4, value: tooltipValue });
                  links.push({ source: parentAccountName, target: othersId, value: t('influenceMapReport.expandCollapseHint'), lineStyle: { width: scaleValue(sumAmount, [minAmount, maxAmount], [1, 8]) } });
              }
              ...
              incomingToShow.forEach(delegation => {
                  ...
                  if (!nodeMap.has(childAccountName)) {
                      const position = nodePositions.get(childAccountName);
                      nodes.push({ 
                          id: childAccountName, name: `@${childAccountName}`, symbolSize: childNodeSize, category: 1, value: delegation.vesting_shares,
                          ...(position && { x: position.x, y: position.y, fixed: true })
                       });
                      nodeMap.set(childAccountName, nodes[nodes.length - 1]);
                      queue.push(childAccountName);
                  }
                  links.push({ source: childAccountName, target: parentAccountName, value: delegation.vesting_shares, lineStyle: { width: scaleValue(parseFloat(delegation.amount), [minAmount, maxAmount], [1, 8]) } });
              });
              ...
              if (incoming.length > incomingToShow.length) {
                  ...
                  nodes.push({ id: othersId, name: `${remaining.length}+ ${t('analyticsDashboard.others')}`, symbolSize: scaleValue(sumAmount, [minAmount, maxAmount], [10 * mobileSizeMultipler, 25 * mobileSizeMultipler]), category: 2, value: tooltipValue });
                  links.push({ source: othersId, target: parentAccountName, value: t('influenceMapReport.expandCollapseHint'), lineStyle: { width: scaleValue(sumAmount, [minAmount, maxAmount], [1, 8]) } });
              }
          }
          finalNodes = nodes;
          finalLinks = links;
      ...
      const usedCategoryIndices = new Set(finalNodes.map(node => node.category));
      const currentActiveCategories = allPossibleCategories.filter((_, index) => usedCategoryIndices.has(index));
      setActiveLegendCategories(currentActiveCategories);

  }, [accountName, chartDimensions, expandedNodes, dataCache, fullIncoming, fullOutgoing, isFullyExpanded, loadingNodes, nodePages, t, allPossibleCategories, nodePositions, isMobile]);
  ...
  const handleLayoutEnd = useCallback(() => {
    ...
    if (!seriesArray || seriesArray.length === 0 || !seriesArray[0].data) return;
    ...
    setNodePositions(prevPositions => {
        ...
        if (Array.from(newPositions.entries()).every(([id, pos]) => 
            prevPositions.has(id) && prevPositions.get(id)?.x === pos.x && prevPositions.get(id)?.y === pos.y
   ...
   */
  const handleNodeClick = useCallback(async (params: any) => {
    if (params.dataType !== 'node' || isFullyExpanded) return;
    const clickedNodeId = params.data.id as string;
    if (loadingNodes.has(clickedNodeId) || accountToFetch === clickedNodeId) return;
    ...
    if (clickedNodeId.startsWith('others-')) {
        ...
        setNodePages(prev => {
            ...
            const current = newPages.get(parentAccount) || { incoming: 1, outgoing: 1 };
  ...
  }, [isFullyExpanded, expandedNodes, dataCache, accountName, loadingNodes, accountToFetch]);
  ...
  if (!data.incoming || !data.outgoing) {
    return <div className="flex justify-center items-center w-full h-full"><Loader2 className="animate-spin h-12 w-12" /></div>;
  }
  ...
  return (
    <div ref={containerRef} className="w-full h-full relative">
      {canExpand && (
        <Button variant="outline" size="sm" onClick={toggleExpandAll} className="absolute top-2 right-2 z-10">
          {isFullyExpanded ? t('analyticsDashboard.collapseAll') : t('analyticsDashboard.expandAll')}
        </Button>
      )}
      <ReactECharts
        ref={chartRef}
        option={getChartOption(graphData.nodes, graphData.links, isFullyExpanded, allPossibleCategories, activeLegendCategories, t, theme)}
        onEvents={onEvents}
