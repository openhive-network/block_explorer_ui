...
const InfluenceMapReport: React.FC<InfluenceMapReportProps> = ({
  ...
  // CSV export of all incoming + outgoing delegations (translated headers).
  const exportDatasets = useMemo(() => {
    ...
    const rowFor = (
      direction: string,
      account: string,
      d: Explorer.VestingDelegation
    ) => ({
      [t("influenceMapReport.colDirection")]: direction,
      [t("influenceMapReport.colAccount")]: account,
      [t("influenceMapReport.colHp")]: Number(
        grabNumericValue(d.vesting_shares).toFixed(3)
      ),
      [t("influenceMapReport.colVests")]: parseFloat(d.amount),
    });
    return [
      {
        name: t("influenceMapReport.exportCsv"),
        filename: `${spacesToUnderscores(
          t("analyticsDashboard.influenceMapReportTitle")
        )}_${accountName}`,
        rows: [
          ...incoming.map((d) =>
            rowFor(t("influenceMapReport.legendIncoming"), d.delegator, d)
          ),
          ...outgoing.map((d) =>
            rowFor(t("influenceMapReport.legendOutgoing"), d.delegatee, d)
          ),
        ],
      },
    ];
  }, [data.incoming, data.outgoing, accountName, t]);
