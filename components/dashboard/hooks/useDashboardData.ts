...
export function useDashboardData(widgets: ActiveWidgetList) {
  ...
  const { witnessesData, isWitnessDataLoading } = useWitnesses(
    config.witnessesPerPages.home, "rank", "asc", isWitnessesActive
  );

  const { communities: popularCommunitiesData, isLoading: isCommunitiesLoading } = useCommunities(
    "", "rank", config.popularCommunitiesCount,  isCommunitiesActive
  );
  ...
  const { opcount, trxOpsLength } = useMemo(() => {
    ...
    const trx_in_block = blockOperations?.operations_result.reduce(
      (max, op) => (typeof op?.trx_in_block === "number" ? Math.max(max, op.trx_in_block) : max),
      0
    );
    return {
