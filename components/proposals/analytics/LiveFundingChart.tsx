...
export const LiveFundingChart = () => {
  ...
  const { chartData, fundingThresholdInHp } = useMemo(() => {
    ...
    const thresholdVests = returnProposal
      ? parseFloat(returnProposal.total_votes)
      : 0;
    ...
    const otherProposals = proposalsData
      ...
      .map((p) => {
        ...
        return {
          ...
          permlink: p.permlink,
          percentage:
            thresholdVests > 0 ? (votesVests / thresholdVests) * 100 : 0,
          isFunded: votesVests >= thresholdVests,
          votesInHpFormatted: convertVestsToHP(
            hiveChain,
            p.total_votes,
    ...
    const thresholdInHp = convertVestsToHP(
      hiveChain,
      thresholdVests.toString(),
      dynamicGlobalData.headBlockDetails.rawTotalVestingFundHive,
      dynamicGlobalData.headBlockDetails.rawTotalVestingShares
    );
            ...
            >
              <a
                ...
                onClick={(e) => {
                  e.preventDefault();
                  document
                    .querySelector(`#proposal-${proposal.id}`)
                    ?.scrollIntoView({
                      behavior: "smooth",
                      block: "center",
                    });
          ...
          <div className="flex items-center flex-wrap justify-end gap-x-4 gap-y-1 text-xs text-slate-500">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="whitespace-nowrap">
                {t("proposalAnalytics.fundingThreshold")}:{" "}
                <strong>{fundingThresholdInHp}</strong>
              </span>
            </div>
          </div>
        </div>
